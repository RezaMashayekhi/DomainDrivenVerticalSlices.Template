...
- **AuditableEntityInterceptor**: Automatically populates `CreatedAt`, `CreatedBy`, `ModifiedAt`, and `ModifiedBy` fields on `BaseAuditableEntity` instances during `SaveChangesAsync`.
- **DispatchDomainEventsInterceptor**: Collects and dispatches domain events from `BaseEntity` instances after successful persistence.
...
- EF Core interceptors for audit fields and domain events
